      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length === 0) {
          ...
          details.participants.forEach(participant => {
            participantsHTML += `<li><span class="participant-email">${participant}</span><button class="delete-btn" data-activity="${name}" data-email="${participant}" title="Remove participant">X</button></li>`;
          });
          participantsHTML += '</ul>';
        }
